...
def cmd_rebuild_aliases(args):
    ...
    """
    unique_aliases = OnceList('Alias')
    lines = _get_initial_alias_file_lines(unique_aliases)
    commands = get_command_dictionary()
    _add_aliases_for_commands(commands, lines, unique_aliases)
    _add_help_alias(commands, lines, unique_aliases)
    with open(settings.ALIAS_FILE, 'w') as f:
        for line in lines:
            f.write(line + '\n')


def _get_initial_alias_file_lines(unique_aliases):
    """
    Returns the initial lines for the alias file.
    """
    unique_aliases.add('manage.reload_aliases')
    unique_aliases.add('manage.realias')
    return [
...
def _add_aliases_for_commands(commands, lines, unique_aliases):
    """
    Build the individual command aliases.
    """
    lines.append('# Individual commands:')
    for name, command in commands.items():
        lines.extend(_expand_aliases(name, command, unique_aliases))


def _expand_aliases(fn_name, command, unique_aliases):
    """
...
def _add_help_alias(commands, lines, unique_aliases):
    """
    Build the alias lines comprising the help function
    """
    unique_aliases.add('help')
    lines.extend([
        ' ',
        'function c.help {',
        '  echo " "',
    ...
    all_aliases = {
        ...
        'manage.realias': 'Rebuilds and reloads aliases in current session.',
        'manage.reload_aliases': 'Reloads aliases in current session.'
    }
